    ...
    <section id="work-process" className="work-process">
      <div className="container" data-aos="fade-up">
        ...
        <div key={wp.id} className="row content">
          <div className={`col-md-5 ${wp.id % 2 === 0 ? 'order-1 order-md-2' : null }`} data-aos={wp.id % 2 === 0 ? 'fade-left' : 'fade-right'}>
            <img src={wp.image} className="img-fluid" alt="" />
          </div>
          <div className={`col-md-7 pt-3 ${wp.id % 2 === 0 ? 'order-2 order-md-1' : null}`} data-aos={wp.id % 2 === 0 ? "fade-right" : "fade-left"}>
            <h3>{wp.name}</h3>
